...
namespace Helper
{
    ...
    public class GoogleSheet
    {
    	...
    	public string spreadsheetId; // If this is a fixed value could also be setup on the webservice to save POST request size.
    	public string servicePassword;
    	public float timeOutLimit = 100f;
    	...
    	public enum QueryType
    	{
			...
			getTableRows, // Returns all rows as arrays and first column will be key of array from the spreadsheet.
			getTableAllRows, // Returns all columns with arrays
    		// Update
    	...
    	public CallBackEventRaw rawResponseCallback = new CallBackEventRaw();
    	...
    	public void CreateObject(Dictionary<string, string> fields, string objTypeName)
    	{
    	...
    	/// <param name="objTypeName">Name of the table that will hold the object.</param>
    	public void CreateObject(string jsonObject, string objTypeName)
    	{
    	...
    	public void CreateTable(string[] headers, string tableTypeName)
    	{
    		Dictionary<string, string> form = new Dictionary<string, string>();
    	...
    	public void GetObjectsByField(string objTypeName, string searchFieldName, string searchValue)
    	{
    		Dictionary<string, string> form = new Dictionary<string, string>();
    	...
    	public void GetTable(string tableTypeName, bool horizontalAxis = true)
    	{
    		Dictionary<string, string> form = new Dictionary<string, string>();
    		form.Add("action", QueryType.getTable.ToString());
    		form.Add("type", tableTypeName);
			form.Add("horizontalAxis", horizontalAxis ? "true" : "false");
    	...
    	/// <summary>
    	public void GetAllTables(bool runtime = true)
    	{
    		Dictionary<string, string> form = new Dictionary<string, string>();
    		form.Add("action", QueryType.getAllTables.ToString());
            // form.Add("action", "Ping");

    		SendRequest(form);
    	}

		public void GetTableRows(string tableTypeName)
		{
    		Dictionary<string, string> form = new Dictionary<string, string>();
    		form.Add("action", QueryType.getTableRows.ToString());
    		form.Add("type", tableTypeName);

    		SendRequest(form);
		}

        public void GetTableAllRows(string tableTypeName)
        {
            Dictionary<string, string> form = new Dictionary<string, string>();
            form.Add("action", QueryType.getTableAllRows.ToString());
            form.Add("type", tableTypeName);
        ...
        /// <summary>
        /// Update spreadsheet from string tables
        /// <summary>
        public void UpdateTables(string objTypeName, List<StringTableVariable> stringTable)
        {
            Dictionary<string, string> form = new Dictionary<string, string>();
            form.Add("action", QueryType.updateTables.ToString());
            form.Add("type", objTypeName);
            form.Add("stringTable", JsonUtility.ToJson(stringTable));
    	...
    	public void UpdateObjects(string objTypeName,
    		...
    		bool runtime = true )
    	{
    		...
    		form.Add("updateField", fieldNameToUpdate);
    		form.Add("updateValue", updateValue);
    	...
    	/// <param name="searchFieldName">Name of the field to search by.</param>
    	/// <param name="searchValue">Value to search for.</param>
    	public void DeleteObjects(string objTypeName, string searchFieldName, string searchValue)
    	{
    		Dictionary<string, string> form = new Dictionary<string, string>();
    	...
    	public void UnpackJson(string response)
    	{
    	...
    	private void SendRequest(Dictionary<string, string> form)
    	{
    	...
    	private void EditorUpdate()
    	{
    		while (!www.isDone)
    		{
    			...
    			{
    				ProcessResponse("TIME_OUT", (float)elapsedTime);
#if UNITY_EDITOR
    				EditorApplication.update -= EditorUpdate;
#endif
    			}
    		...
    		{
    			ProcessResponse(MSG_CONN_ERR + "Connection error after " + elapsedTime.ToString() + " seconds: " + www.error, (float)elapsedTime);
    			return;
    		}
    	...
    	public void ProcessResponse(string response, float time)
    	{
